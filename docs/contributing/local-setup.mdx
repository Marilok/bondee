...
| [pnpm](https://pnpm.io/installation) | ≥ 12.4.0 | Declared in root `package.json` `packageManager` / `engines.pnpm`. Install it yourself — the lockfile does not download pnpm (`pmOnFail: ignore`). |
...
`BONDERY_PRIVATE_REDIS_URL` is **required** in development and production. See [Installation](../deploy/installation.mdx).
...
Inbound Stripe events (portal cancel, payment failure, trial-ending email) do not reach `127.0.0.1`. After [Stripe CLI](https://docs.stripe.com/stripe-cli) is installed:
...
See [Environment configuration](environment.mdx#stripe-cli-local-webhooks).
