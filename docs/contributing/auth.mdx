...
Webapp `/login` and `/oauth/login` also offer a **passwordless sign-in email** (Better Auth `magicLink` plugin — click the emailed link; no OTP, no passwords). First verify can create the user (`disableSignUp: false`). Mobile login is unchanged (GitHub / LinkedIn).

```
User → client (webapp / mobile / extension)
     → API /auth/oauth2/authorize
     → API login (sign-in email, GitHub/LinkedIn, or passkey)
...
Set `BONDERY_PRIVATE_REDIS_URL=redis://127.0.0.1:26636` in root `.env.local` (default in examples), then `pnpm run env`.
...
Your extension ID changed or the OAuth client was wiped (e.g. after `prisma migrate reset`). Update `BONDERY_INFRA_CHROME_EXTENSION_ID`, run `pnpm run env`, then `pnpm run provision:oauth-clients`. See [Chrome Extension OAuth Setup](../../.agents/workflows/CHROME-EXTENSION-OAUTH.md).

### `invalid_target` / requested resource is not configured

The authorization server matches the RFC 8707 `resource` parameter to `oauth_resource.identifier` exactly. `localhost` and `127.0.0.1` are different identifiers. Local env is usually `http://localhost:26631`; Chrome identity fetches `http://127.0.0.1:26631`. Re-run `pnpm run provision:oauth-clients` so both loopback aliases are registered and linked, then restart the API. The extension must still send `resource` = `BONDERY_PUBLIC_API_URL` (not the rewritten fetch host).
...
| Chrome extension token exchange (`Failed to exchange code for tokens` after API `POST /auth/oauth2/token` 200) | Local token fetch uses `127.0.0.1` from `chrome-extension://<id>`. The API allows `chrome-extension://{BONDERY_INFRA_CHROME_EXTENSION_ID}`. Restart the API and reload the unpacked extension so `host_permissions` includes `http://127.0.0.1:<api-port>/*`. |
